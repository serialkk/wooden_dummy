...
HBITMAP KObject::RotateBitmap( HDC hdc, float fAngle, int iSizeWidth, int iSizeHeight)
{
	// 회전 될 DC 및 비트맵 생성
...
void KObject::SetPos( float iX, float iY )
{
	m_fPosition[0] = iX;
	m_fPosition[1] = iY;
...
void KObject::SetObjDC(HDC hdc)
{
	m_hDrawDC = hdc;
	//m_Image.SetDC( hdc );
}
void KObject::CreateOffScreenDC(HDC hdc, int iWidth, int iHeight)
{
...
bool KObject::Load( TCHAR* strLoadFile, 
...
//void KObject::DrawBitmap(	int iX, int iY,
...
void KObject::DrawBitmap(	HDC hdc, 
							int iX, int iY,
							int iWidth, int iHeight,
							int iStartX, int iStartY,
							HBITMAP bitmap,
