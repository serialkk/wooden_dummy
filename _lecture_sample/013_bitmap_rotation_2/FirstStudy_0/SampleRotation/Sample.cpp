	...
	float fAngle, int iWidth, int iHeight)
{
	...
	HBITMAP old = (HBITMAP)SelectObject(m_RotationBitmapDC, bitmap);
	...
	SelectObject(m_RotationBitmapDC, old);
...
void Sample::DrawRotate(float fAngle)
{
	...
	if (DeleteDC(m_hMaskMemDC)== FALSE)
	{
		return;
	}
	if (DeleteDC(m_hColorMemDC) == FALSE)
	{
		return;
	}
}
