...
float TMapLOD::GetExpansionRatio( D3DXVECTOR3 vCenter, float fRadius )
{
	float fRatio = 0.0f;
...
float TMapLOD::GetExpansionRatio( D3DXVECTOR3 vCenter )
{
	float fDistance = D3DXVec3Length(&(*m_pCamera->GetEyePt() - vCenter));
	if (fDistance > m_pCamera->m_fFarPlane)
	{
...
float TMapLOD::GetExpansionRatio( D3DXVECTOR3 vMax, D3DXVECTOR3 vMin )
{
	float fRatio = 0.0f;
	D3DXVECTOR3 vCenterViewPos, vSideViewPos;
